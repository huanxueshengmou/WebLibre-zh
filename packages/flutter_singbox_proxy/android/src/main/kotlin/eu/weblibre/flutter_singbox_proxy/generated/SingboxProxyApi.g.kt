...
val SingboxProxyApiPigeonMethodCodec = StandardMethodCodec(SingboxProxyApiPigeonCodec())


/** Generated interface from Pigeon that represents a handler of messages from Flutter. */
interface SingboxProxyApi {
  suspend fun validateProfile(profile: SingboxProxyProfile): String?
  suspend fun buildConfig(profiles: List<SingboxProxyProfile>, options: SingboxProxyRuntimeOptions): SingboxProxyConfigResult
  suspend fun start(profiles: List<SingboxProxyProfile>, options: SingboxProxyRuntimeOptions): SingboxProxyRuntimeState
  suspend fun stop(profileIds: List<String>)
  suspend fun stopAll()
          ...
          channel.setMessageHandler { message, reply ->
            ...
            CoroutineScope(Dispatchers.Main).launch {
              val wrapped: List<Any?> = try {
                listOf(api.validateProfile(profileArg))
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            CoroutineScope(Dispatchers.Main).launch {
              val wrapped: List<Any?> = try {
                listOf(api.buildConfig(profilesArg, optionsArg))
              } catch (exception: Throwable) {
                SingboxProxyApiPigeonUtils.wrapError(exception)
              }
              reply.reply(wrapped)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            CoroutineScope(Dispatchers.Main).launch {
              val wrapped: List<Any?> = try {
                listOf(api.start(profilesArg, optionsArg))
              } catch (exception: Throwable) {
                SingboxProxyApiPigeonUtils.wrapError(exception)
              }
              reply.reply(wrapped)
...
private class SingboxProxyApiPigeonStreamHandler<T>(
    val wrapper: SingboxProxyApiPigeonEventChannelWrapper<T>
...
interface SingboxProxyApiPigeonEventChannelWrapper<T> {
...
abstract class StreamStateStreamHandler : SingboxProxyApiPigeonEventChannelWrapper<SingboxProxyRuntimeState> {
  companion object {
    fun register(messenger: BinaryMessenger, streamHandler: StreamStateStreamHandler, instanceName: String = "") {
      var channelName: String = "dev.flutter.pigeon.flutter_singbox_proxy.SingboxProxyEventsApi.streamState"
      if (instanceName.isNotEmpty()) {
        channelName += ".$instanceName"
      }
      val internalStreamHandler = SingboxProxyApiPigeonStreamHandler<SingboxProxyRuntimeState>(streamHandler)
      EventChannel(messenger, channelName, SingboxProxyApiPigeonMethodCodec).setStreamHandler(internalStreamHandler)
    }
  }
// Implement methods from SingboxProxyApiPigeonEventChannelWrapper
override fun onListen(p0: Any?, sink: PigeonEventSink<SingboxProxyRuntimeState>) {}

override fun onCancel(p0: Any?) {}
}
      
abstract class StreamLogsStreamHandler : SingboxProxyApiPigeonEventChannelWrapper<SingboxProxyLogMessage> {
  companion object {
    fun register(messenger: BinaryMessenger, streamHandler: StreamLogsStreamHandler, instanceName: String = "") {
      var channelName: String = "dev.flutter.pigeon.flutter_singbox_proxy.SingboxProxyEventsApi.streamLogs"
      if (instanceName.isNotEmpty()) {
        channelName += ".$instanceName"
      }
      val internalStreamHandler = SingboxProxyApiPigeonStreamHandler<SingboxProxyLogMessage>(streamHandler)
      EventChannel(messenger, channelName, SingboxProxyApiPigeonMethodCodec).setStreamHandler(internalStreamHandler)
    }
  }
// Implement methods from SingboxProxyApiPigeonEventChannelWrapper
