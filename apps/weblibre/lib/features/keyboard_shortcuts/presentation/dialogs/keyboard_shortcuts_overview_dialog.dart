...
class KeyboardShortcutsOverviewDialog extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AlertDialog(
      title: Text(tr("Keyboard Shortcuts")),
      content: SizedBox(
        width: 520,
        child: bindings.isEmpty
            ? Text(
                settings.enabled
                    ? tr("No keys are assigned to browser actions.")
                    : tr("Keyboard shortcuts are switched off."),
              )
            : ListView(
                shrinkWrap: true,
                children: [
      ...
      actions: [
        TextButton(onPressed: customize, child: Text(tr("Customize"))),
        FilledButton(
