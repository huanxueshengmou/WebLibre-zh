...
class _DownloadDirectoryTile extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subtitle = switch (directoryUri) {
      null => tr("Saving to the system Downloads folder"),
      final uri when unavailable =>
        tr("No longer available — saving to the system Downloads folder ({0})", [describeDownloadTreeUri(uri)]),
    ...
    return ListTile(
      ...
      subtitle: Text(
        useExternalDownloadManager
            ? tr("The download manager app chooses where files are saved")
            : subtitle,
      ),
      leading: Icon(
      ...
      trailing: directoryUri != null && !useExternalDownloadManager
          ? IconButton(
              ...
              tooltip: tr("Use the system Downloads folder"),
              onPressed: reset,
            )
          : null,
