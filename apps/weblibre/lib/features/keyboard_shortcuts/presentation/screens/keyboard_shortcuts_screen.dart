...
class KeyboardShortcutsScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> restoreDefaults() async {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: Text(tr("Restore default shortcuts?")),
          content: Text(
            tr("Every action goes back to its Firefox default keys. Your changes are lost."),
          ),
          actions: [
            TextButton(
    ...
    return SettingsCustomScrollScaffold(
      ...
      searchHintText: tr("Search actions or keys"),
      actions: [
        if (settings.enabled && settings.hasCustomizations)
          IconButton(
            icon: const Icon(Icons.restart_alt),
            tooltip: tr("Restore default shortcuts"),
      ...
      slivers: [
          ...
          sliver: SliverToBoxAdapter(
            child: Card.filled(
              ...
              child: SwitchListTile.adaptive(
                ...
                subtitle: Text(
                  tr("Browser actions on a hardware keyboard, even while a page has focus"),
                  style: TextStyle(
        ...
        if (settings.enabled)
          if (byCategory.isEmpty)
            SliverFillRemaining(
              hasScrollBody: false,
              child: Center(child: Text(tr("No matching actions."))),
            )
          else
            SliverPadding(
